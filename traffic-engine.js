/* NEMA Drive Navigation - Traffic Intelligence Engine v2
 * Provider-neutral traffic state, incidents, freshness, confidence and route impact.
...
(function(){'use strict';
 ...
 function normalize(x){if(!x)return null;const updatedAt=num(x.updatedAt,now()),source=x.source||'unknown',live=x.live===true&&String(source).toLowerCase()!=='demo';const level=levels.has(String(x.level||'').toLowerCase())?String(x.level).toLowerCase():'unknown';return{level,delaySec:Math.max(0,num(x.delaySec,0)),speedKmh:num(x.speedKmh),source,confidence:x.confidence||'unknown',updatedAt,expiresAt:num(x.expiresAt),incidents:Array.isArray(x.incidents)?x.incidents.map(normalizeIncident).filter(Boolean):[],live};}
 function ingest(x){state.current=normalize(x);state.incidents=state.current?.incidents||[];state.updatedAt=now();if(typeof window!=='undefined'&&window.NEMANavigation?.setTraffic)window.NEMANavigation.setTraffic(state.current);return state.current;}
 ...
 function registerProvider(name,adapter){if(!name||!adapter||typeof adapter!=='object')return false;state.providers[name]=adapter;return true;}
